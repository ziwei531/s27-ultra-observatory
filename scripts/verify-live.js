...
const base = process.env.SITE_URL ?? "https://ziwei531.github.io/galaxy-leak-observatory/";
...
for ( const model of catalog.models ) {
	const manifestText = await ( await fetchLive( model.manifest ) ).text();
	assert.equal( manifestText, await readFile( model.manifest, "utf8" ) );
	...
	if ( manifest.timeline ) {
		assert.equal( await ( await fetchLive( manifest.timeline ) ).text(), await readFile( manifest.timeline, "utf8" ) );
	}
}
for ( const path of [ "styles/site.css", "js/app.js", "js/model.js", "assets/optical-study.svg", "assets/favicon.svg" ] ) {
	await fetchLive( path );
}
console.log( `LIVE VERIFIED: ${ base } commit=${ build.commit }; HTML, model manifests, snapshots and assets passed` );
